...
local enableRED = 1 -- 1 = RED SAMs use IADS, 0 = RED SAMs do NOT use IADS
local enableBLUE = 1 -- 1 = BLUE SAMs use IADS, 0 = BLUE SAMs do NOT use IADS
...
local RedSAMSites = {}
local RedEWRSites = {}
local RedAEWAircraft = {}

local BlueSAMSites = {}
local BlueEWRSites = {}
local BlueAEWAircraft = {}
...
local function disableAllSAMs()
  for i, SAM in pairs(RedSAMSites) do
    disableSAM(SAM)
  end
  for i, SAM in pairs(BlueSAMSites) do
    disableSAM(SAM)
  end
end
...
local function associateSAMS()  
  for j, SAM in pairs(RedSAMSites) do  
    SAM.ControlledBy = {}
  end
  for i, EWR in pairs(RedEWRSites) do
  EWR.SAMsControlled = {}
    for j, SAM in pairs(RedSAMSites) do
  ...
  for j, SAM in pairs(BlueSAMSites) do  
    SAM.ControlledBy = {}
  end
  for i, EWR in pairs(BlueEWRSites) do
  EWR.SAMsControlled = {}
    for j, SAM in pairs(BlueEWRSites) do
      if getDistance3D(SAM.Location, EWR.Location) < EWRAssociationRange then        
        EWR.SAMsControlled[SAM.Name] = SAM
        SAM.ControlledBy[EWR.Name] = EWR
      end
...
--            env.info("For unit: ")
--            env.info(contact.object:getName())   
            if (contact.object:getCategory() == 1) and contact.object:inAir() and contact.object:getCoalition() == 1 then  
--              env.info("Is hostile air threat")      
              EWR.hostileAirDetected[contact.object:getName()] = contact      
--              env.info("added to list")      
            elseif contact.object:getCategory() == 2 and contact.object:getCoalition() == 1 and contact.object:getDesc().guidance == 5  and IADSEWRARMDetection == 1  then
              local prevDetected = 0           
--              env.info("Hostile ARM detected by EWR")
--              env.info("EWR has detected: "..tablelength(EWR.hostileARMDetected).." hostile ARMS")
              if tablelength(EWR.hostileARMDetected) > 0 then            
...
--            env.info("For unit: ")
--            env.info(detectedTarget.object:getName())                
            if (detectedTarget.object:getCategory() == 1) and detectedTarget.object:inAir() and detectedTarget.object:getCoalition() == 1 then           
              SAM.hostileAirDetected[detectedTarget.object:getName()] = detectedTarget 
--              env.info("Detected target is hostile air")
            elseif detectedTarget.object:getCategory() == 2 and detectedTarget.object:getCoalition() == 1 and detectedTarget.object:getDesc().guidance == 5 and SAM.Type ~= "Tor 9A331" and IADSSAMARMDetection == 1 then
              local prevDetected = 0
--              env.info("Hostile ARM detected by SAM")
--              env.info("SAM has detected: "..tablelength(SAM.hostileARMDetected).." hostile ARMS")
              if tablelength(SAM.hostileARMDetected) > 0 then            
  ...
  if event.id == world.event.S_EVENT_SHOT and IADSRadioDetection == 1 then
    if event.weapon then    
      ...
      if WeaponDesc.guidance == 5 then      
        for i, SAM in pairs(RedSAMSites) do        
          if math.random(1,100) < IADSARMHidePctage and getDistance(SAM.Location, WeaponPoint) < IADSARMHideRangeRadio and ordnance:getCoalition() == 2 then          
            magnumHide(SAM)            
          end        
        end
        for i, SAM in pairs(BlueSAMSites) do        
          if math.random(1,100) < IADSARMHidePctage and getDistance(SAM.Location, WeaponPoint) < IADSARMHideRangeRadio and ordnance:getCoalition() == 1 then          
            magnumHide(SAM)            
          end        
        end      
      end
  ...
  if event.initiator:getCategory() == Object.Category.UNIT and event.initiator:getGroup() then
    ...
    for i, SAM in pairs(RedSAMSites) do     
      if eventGroup:getName() == SAM.Name then
        ...
        if SAM.numSAMRadars < 1 then
          for j, EWR in pairs(RedEWRSites) do           
    ...
    for i, SAM in pairs(BlueSAMSites) do     
      if eventGroup:getName() == SAM.Name then
        ...
        if SAM.numSAMRadars < 1 then
          for j, EWR in pairs(BlueEWRSites) do           
              for k, SAMControlled in pairs(EWR.SAMsControlled) do 
                if SAMControlled.Name == SAM.Name then
                  SAMControlled = nil
                end              
